...
The microcontroller for this project had to be able to do a view things: connect to the school network which is an enterprise network, send data over wifi, have the necessary io pins to read out all the sensors and be low power.
At first the ESP8266 was considerd as this is an easy to use microcontroller. The problem with this microcontroller is that it can't connect to the wifi network at school as it doesn't support enterprise networks.
Because of this the ESP32 was used, this is the 'bigger' brother of the ESP8266 that supports the enterprise connection. To be more specific the single core vresion of this ESP32 is used because it uses less power then the normal two core model.
...
Then there are also interrupt pins those are recognisable by the int in the name. The detector from the microphone is labeled Detector\_out and is connected to a pin that is internally connected to an ADC.
...
%\end{figure}
\includepdf[pages=-]{fig/schematic drawing sensorboard.pdf}
\newpage
...
%	\includegraphics[width=1\linewidth]{schematic drawing sensorboard.pdf}
%\end{figure}
\includepdf[pages=-]{fig/Pricing information PI-1200952.pdf}
\newpage
...
%	\includegraphics[width=1\linewidth]{schematic drawing sensorboard.pdf}
%\end{figure}
\includepdf[pages=-]{fig/Pricing information PI-1200953.pdf}
\newpage
\end{document}