...
To detect human presence, we have decided to use an infrared sensor. The human body produces quite some body heat that is emitted. The AMG8833 is an 8x8-pixel IR-sensor, that maps the heat per pixel. This allows us to determine how many people are presence in a certain area. For example we can determine if there is a single person present or a group of persons in a row.
\\ \\
One of the considerations that has to be studied is the effect of distance, ambient temperature and the field of view of the sensor. The sensor has a total angle view of \SI{60}{\degree} for the horizontal and vertical axis of the pixel grid. Depending on the height, the area that the sensor covers, changes. If the height increases, the area also increases. Not only the field of view is influenced by the height but also the result of the measurements. The further the heat source is from the sensor, the lower the measured temperature will be.
\\ \\
...
\begin{itemize}
	...
	\item The nWake-pin is also an active low pin. This pin can be used to put the sensor into sleep just by pulling the pin low.
\end{itemize}
...
\end{itemize}
Conclusion: \\ The first conclusion of these tests is that bringing the sensors out of sleep is the fastest way to get a measurement. This is because for getting the sensor out of sleep we just need to alter the mode. 
...
\\ \\
$ x = height \cdot tan (\SI{30}{\celsius}) = 1.7 m \cdot 0.577 = 0.981 m$
\\ \\
...
For this we have placed the sensor at this height and analysed the results if someone moved beneath the sensor. When analysing the result we found that a person was represented by 1 or 2 pixels with a value that is about 2-\SI{3}{\celsius} higher than the surroundings. The test was performed in a space with an overall temperature of about 17 degrees. Which is not the normal room temperature. During testing the results were fluctuating about \SI{0.25}{\celsius} which is the resolution of the sensor in \SI{}{\celsius}. \\ \\
